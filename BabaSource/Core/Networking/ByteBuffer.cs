...
namespace Core.Networking
{
    public class ByteBuffer : IDisposable
    {
        ...
        public ByteBuffer(string str) : this()
        {
            WriteString(str);
        }

        public ByteBuffer(byte[] str) : this()
        {
            WriteBytes(str);
        }

        public ByteBuffer(bool str) : this()
        {
            WriteBool(str);
        }

        public ByteBuffer(float str) : this()
        {
            WriteFloat(str);
        }

        public ByteBuffer(long str) : this()
        {
            WriteLong(str);
        }

        public ByteBuffer(int str) : this()
        {
            WriteInt(str);
        }

        public ByteBuffer(short str) : this()
        {
            WriteShort(str);
        }

        public ByteBuffer(NetworkStream stream) : this()
        {
        ...
        public void WriteBytes(byte[] _input, int? count = null)
        {
            if (count != null)
            {
                buffer.AddRange(_input.Take((int)count));
            }
            else
            {
        ...
        public string ReadString(bool _peek = true)
        {
            try
            {
                ...
                string _value = Encoding.ASCII.GetString(readBuffer, readPos, _length).TrimEnd('\0'); // Convert the bytes to a string
                if (_peek & buffer.Count > readPos)
                {
                    // If _peek is true and there are unread bytes
                    if (_value.Length > 0)
